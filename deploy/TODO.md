...
| **统一时光轴数据** | `timeline_api/router.py` → `timeline.js` | 管理员事件与行程统一读取 fan-hub 事件/行程 CSV，不再维护网站专用手工事件文件 |
...
- 管理员新增事件或行程时，通过 fan-hub 管理员反馈流程更新 `schedule_record/chenjiayi_events.csv`，并同步兼容副本 `schedule.csv`。
...
事件/行程、社交时间轴、回放汇总或直播封面数据更新后需要同步到阿里云。生产自动同步由阿里云主动拉取；本地部署工具仍可作为手动兜底触发一次性同步：
