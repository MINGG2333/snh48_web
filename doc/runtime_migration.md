...
> 当前迁移目标：**阿里云公开站迁移到新服务器**（场景 B）。腾讯云非公开站/数据生成主机迁移属于场景 A，不能套用公开站的精简副本规则。
...
- 两端 `/home/snh48_web` 均为提交 `0100002`，已跟踪代码和配置一致；普通动态导出文件的 `generated_at` 会因两端生成时间不同而变化，但去除该时间字段后内容一致。
...
首个正式快照只能作为告警基线，不能单独决定升降配。当前观察到腾讯云非公开站的磁盘余量最紧（根盘已用 89.53%，可用约 5.23 GiB），内存可用约 0.63 GiB，5 分钟负载约 2.79（2 CPU）；阿里云公开站根盘已用 44.09%（可用约 35.33 GiB），内存可用约 5.41 GiB，5 分钟负载约 0.04（4 CPU）。因此现阶段应优先处理腾讯云磁盘增长和内存压力，不能用阿里云的空闲容量反推腾讯云可以缩容。
...
当前阿里云公开站保留 `QA_ENABLED=true`，网站进程 RSS 约 1.45 GiB，知识库和虚拟环境还需要启动、缓存及临时文件空间。因此建议新主机采用 **2 vCPU、8 GiB 内存、100 GiB SSD、4 GiB swap**，并保留独立 IPv4、80/443、密钥 SSH 和至少 5 Mbps（建议 10 Mbps）的公网带宽。短期访问量不增长时，2 vCPU 足够；8 GiB 是为 QA 冷启动和 Python/Chroma 峰值留出的余量。

用户提出的 **2 vCPU、4 GiB、50 GiB、无固定流量包、峰值 200 Mbps、1 个 IPv4** 可以完成代码部署和低流量静态网站运行，但不建议在保持 QA 开启时作为生产规格：4 GiB 会使 QA 模型冷启动、应用进程、Nginx、同步和迁移临时任务争用内存，50 GiB 也几乎没有日志/备份余量；“峰值 200 Mbps”不能弥补 CPU、内存或磁盘不足。若明确关闭 QA、仅运行公开页面，2 vCPU/4 GiB/50 GiB 可作为最低临时配置，但仍应加 2--4 GiB swap，并把归档备份放到外部 COS/对象存储。

新机上线后应保留旧阿里云节点 48--72 小时；先完成只读烟测和 QA `/api/qa/status`，再切 DNS。连续 30 天统计达到后，才能依据真实峰值决定是否降到 4 GiB。
