...
> 当前迁移计划（2026-09-05）：**暂缓阿里云公开站迁移**，先保留现网并连续观察 30 天容量数据，再决定新主机的最低规格。观察窗口暂定为 2026-09-05 至 2026-10-05；腾讯云非公开站/数据生成主机迁移仍属于场景 A，不能套用公开站的精简副本规则。
...
后续规格评估至少使用连续 30 天数据，并分别计算每个节点的：小时请求峰值、页面请求峰值、状态码异常比例、响应字节、CPU 使用率 p95/p99、负载相对 CPU 数、可用内存 p95、Swap 使用趋势、根盘增长速度和 I/O 增量。扩容或迁移前保留原节点至少 48--72 小时作为回滚与对照窗口；任何缩容决定都必须以峰值而非日均值为依据。

### 当前决定：暂缓迁移并完成 30 天容量观察（2026-09-05）
...
观察期内保持现有阿里云规格和 `QA_ENABLED=true`，不切 DNS、不停旧站。至少在低峰期完成 3 次可回滚的 QA 冷启动验收，并同时记录 `/api/qa/status`、一次代表性问答、服务 `NRestarts`、进程 `VmHWM`、`memory.events` 和 swap 使用。只有同时满足以下条件，**2 vCPU / 4 GiB RAM / 80 GiB SSD / 4--8 GiB swap** 才能作为低流量公开站的候选最低规格：

- 三次冷启动均成功，`VmHWM` 不超过约 3 GiB，启动后仍保留至少约 0.75--1 GiB `MemAvailable`；
- 没有 `oom`/`oom_kill`、异常重启或持续增长的 swap，稳定运行 30 分钟后 swap 使用最好回落到 256 MiB 以下；
- 代表性问答和普通页面的延迟、5xx 比例不劣于现网，且没有并发问答导致的明显 I/O 抖动。

这只是“可验证后才允许采用”的成本下限，不是当前承诺。若 QA 要保持随时可用、存在并发问答，仍建议 **2 vCPU / 8 GiB RAM / 100 GiB SSD / 4 GiB swap**；若新站明确关闭 QA，2 vCPU / 4 GiB 可以运行静态和普通 API 页面。原先的 50 GiB 磁盘方案不建议保留，因为知识库、虚拟环境、日志、备份和同步临时文件会挤压长期余量。观察期结束后再依据两端各自的 p95/p99 峰值决定是否降配或迁移。
