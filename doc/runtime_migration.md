...
- 腾讯云已启用 sysstat 定时采样，`/var/log/sa/sa01`、`sa02`、`sa03` 等文件可回看 CPU、内存、I/O 和负载。最近快照显示根分区约 92.46%（50G 中可用约 3.76G），需要优先关注磁盘增长和日志/缓存清理；2GiB 物理内存主机当前依靠约 12GiB swap 缓冲，不能把它当成正常容量余量。
