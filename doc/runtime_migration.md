...
- 两台服务器均保留 Nginx 原始访问日志并按日轮转。当前可见的专用站点日志约为：腾讯云 `2026-08-24` 至 `2026-09-03`（约 3,700 个请求），阿里云 `2026-08-20` 至 `2026-09-03`（约 32,800 个请求）；轮转文件约保留 14 天，因此这不是“自首次部署以来”的完整历史。这里的请求数包含静态资源、爬虫和 4xx/5xx，不能直接当作 PV/UV；日志包含请求、状态码和时间，但没有现成的跨域 PV/UV 报表。
...
- `website/data/interaction_logs/`、OB 访问记录和 systemd journal 也会保留应用层/访客观测信息，但它们不是按天汇总的容量数据。结论是：**已有原始记录，尚无覆盖部署全周期、同时对齐 PV/UV、CPU、内存、磁盘和 I/O 的长期容量报表**。当前服务器规格是否足够可以做粗略判断，不能据此严谨地压缩或扩容。
- 后续若要用数据定规格，建议保留更长的 Nginx 汇总（每日请求数、状态码、响应时间、独立匿名访客数）并在两台机器启用统一周期的资源快照；迁移时把汇总 CSV/报告列入第六类观测与回滚材料，而不是复制到业务共享状态。
