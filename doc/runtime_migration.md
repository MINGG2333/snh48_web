...
> 当前迁移计划（2026-09-05）：**暂缓阿里云公开站迁移**，先保留现网并观察至 2026-10-01，再决定新主机的最低规格；迁移实施窗口安排为 2026-10-01 至 2026-10-05，共五天。腾讯云非公开站/数据生成主机迁移仍属于场景 A，不能套用公开站的精简副本规则。
...
## 双服务器一致性审计（2026-09-05）

本次核查同时比较了网站 Git 工作树、两端有效配置、共享状态、fan-hub 网站数据副本和可靠待处理箱：
...
观测告警的节点显示名也已统一为“腾讯云 cjy.plus”和“阿里云 cjy.我爱你”，避免同一 event ID 在两端因旧标签产生内容哈希差异。
...
## 现有访问与资源记录（截至 2026-09-05）

- 两台服务器均保留 Nginx 原始访问日志并按日轮转。**在本次规则部署前**可见的专用站点日志约为：腾讯云 `2026-08-24` 至 `2026-09-03`（约 3,700 个请求），阿里云 `2026-08-20` 至 `2026-09-03`（约 32,800 个请求）；旧规则通常只保留约 10--14 份轮转文件，因此这不是“自首次部署以来”的完整历史。新规则已取消按份数删除，后续由“超过 1 GiB 且 COS 归档校验成功后才删除”的任务控制。首次完整采集后的当前汇总为：腾讯云约 4,588 个请求、2,679 个页面请求；阿里云约 33,555 个请求、10,926 个页面请求（按天 UV 另行记录，不能直接相加为总 UV）。这里的请求数包含静态资源、爬虫和 4xx/5xx，不能直接当作 PV/UV；旧日志没有现成的跨域 PV/UV 报表。
- 腾讯云已启用 sysstat 定时采样，`/var/log/sa/sa01`、`sa02`、`sa03` 等文件可回看 CPU、内存、I/O 和负载。该历史快照显示根分区约 92.46%（50G 中可用约 3.76G），当时约 2GiB 物理内存主机配置约 12GiB swap。2026-09-11 用户确认 QA 停用后的容量调整，改为 4GiB `/swapfile-4g`，释放约 8GiB 磁盘；当前状态见 fan-hub `doc/running_status.md`，不能把历史 swap 容量当成当前配置或正常内存余量。
- 阿里云当前为 4 vCPU、约 7.1GiB 内存、根分区约 44.18% 使用率（约 35.3GiB 可用），但未发现已启用的 `/var/log/sa` 历史采样；只能依靠当前快照、Nginx 日志和 systemd journal 做近期判断。
...
- `snapshots.jsonl`：每次采样的 CPU 使用率（相邻采样差值）、1/5/15 分钟负载、内存/Swap、根盘容量和磁盘 I/O 计数/增量；同时记录 `website_process` 下网站服务的 RSS、启动以来峰值 RSS（`VmHWM`）、进程 swap、虚拟内存和线程数，避免 5 分钟间隔漏掉 QA 冷启动峰值；
...
首个正式快照只能作为告警基线，不能单独决定升降配。最近快照显示腾讯云非公开站根盘已用约 92.46%（可用约 3.76 GiB），内存可用约 0.55 GiB，5 分钟负载约 1.48（2 CPU），swap 已用约 2.5 GiB；阿里云公开站根盘已用约 44.18%（可用约 35.3 GiB），内存可用约 5.2 GiB，5 分钟负载约 1.07（4 CPU），当前没有 swap。因此两端必须分别评估，不能用阿里云的空闲容量反推腾讯云可以缩容。

后续规格评估使用从现在至 2026-10-01 的完整观察窗口，并分别计算每个节点的：小时请求峰值、页面请求峰值、状态码异常比例、响应字节、CPU 使用率 p95/p99、负载相对 CPU 数、可用内存 p95、Swap 使用趋势、根盘增长速度和 I/O 增量。扩容或迁移前保留原节点至少 48--72 小时作为回滚与对照窗口；任何缩容决定都必须以峰值而非日均值为依据。

### 当前决定：观察至 2026-10-01，随后五天完成迁移（2026-09-05）

阿里云 QA 当前为 `ready`。网站进程常驻 RSS 约 **1.05 GiB**，但本次进程启动以来的历史峰值 `VmHWM` 约 **2.32 GiB**；知识库目录约 **3.3 GiB**，虚拟环境约 **5.3 GiB**。这说明日常问答请求不会每次重新加载整套模型，常驻占用确实低于冷启动峰值，但不能只按 1.05 GiB 选择规格。2026-09-04 22:54 的启动日志显示知识库在约 10 秒内加载完成，当前没有 OOM 或异常重启记录。

当前资源快照的覆盖范围仍不足一个月：阿里云 `snapshots.jsonl` 只有 2026-09-03 20:03 UTC 至 2026-09-04 19:12 UTC 的约 24 小时、276 个样本；访问日志汇总已有 2026-08-21 至 2026-09-05 的约 16 天。新快照会额外保存 `website_process.rss_bytes`、`hwm_bytes`、`swap_bytes`、`vm_size_bytes` 和 `threads`，用于保留短暂 QA 冷启动峰值。两节点数据目录始终分开，不能合并计算。

swap **不是云主机规格自动提供的内存**，通常需要在磁盘上自行创建 swapfile 或 swap 分区；配置 8 GiB swap 就会占用约 8 GiB 磁盘。它可以在短时峰值时延缓 OOM，但不能保证服务不崩溃：swap 活跃时会产生明显 I/O 延迟，磁盘满、cgroup 限制、内核 OOM 或并发请求过高时仍可能失败。因此 swap 只作为应急缓冲，不能替代物理内存，也不能用来掩盖持续内存不足。

观察期内保持现有阿里云规格和 `QA_ENABLED=true`，不切 DNS、不停旧站。至少在低峰期完成 3 次可回滚的 QA 冷启动验收，并同时记录 `/api/qa/status`、一次代表性问答、服务 `NRestarts`、进程 `VmHWM`、`memory.events` 和 swap 使用。规格不预设某个内存档位，而是在 10 月 1 日根据“物理内存最小值 + 必要的系统 swap”逐级验证：从云厂商可购买的最低档开始，只接受同时满足以下条件的最小档位：

- 三次冷启动均成功；在候选物理内存档位下，冷启动峰值、系统和其他常驻进程合计后仍保留明确的物理内存余量（建议至少 20%），而不是把物理内存全部交给 QA；
- 没有 `oom`/`oom_kill`、异常重启或持续增长的 swap，稳定运行 30 分钟后 swap 不应持续活跃或引起明显 I/O 等待；
- 代表性问答和普通页面的延迟、5xx 比例不劣于现网，且没有并发问答导致的明显 I/O 抖动。

这不是预先承诺的固定配置，而是“以用户体验为约束、尽量降低物理内存”的选择流程。若较低档位无法通过冷启动或并发验收，就直接升到下一档，不用 swap 掩盖不足；若 QA 明确关闭，则可单独评估更低档位。系统盘仍需覆盖知识库、虚拟环境、日志、备份和同步临时文件，不能只按当前空闲空间倒推容量。10 月 1 日完成规格冻结后，按五天迁移窗口执行：第 1 天备份和建机，第 2 天部署代码/依赖并同步只读数据，第 3 天完成 QA 与双节点只读烟测，第 4 天低流量切换准备和回滚演练，第 5 天 DNS 切换及观察；切换后旧阿里云至少保留 48--72 小时作为回滚节点。
...
现网历史 unit 名称继续保留，以免旧脚本或回滚命令失效；同时安装下面的可读 systemd 别名：
