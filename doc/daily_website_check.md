...
- `chenjiayi_events.csv`、`schedule.csv`、`social_record/timeline/chenjiayi_social_timeline.json`、`manual_events.csv` 两端 hash 一致；四个可写状态按下一节核对 revision，不用普通 rsync 日志作为一致性依据。社交时间轴只同步轻量 JSON，不同步原始社交 CSV、Cookie 或采集器。
