...
- 用户留下邮箱 → 前端发送 `email_submit` 事件 → 写入通知中心
...
管理员应优先登录 `/ob` 查看“可靠待处理箱”。它汇总两台网站服务器收到的投诉和 QA 邮箱请求：

- 蓝色“来源：腾讯云 cjy.plus”表示请求最初由腾讯云接收。
- 橙色“来源：阿里云 cjy.我爱你”表示请求最初由阿里云接收。
- “开始处理”“标记完成”“驳回”会追加不可变状态事件，并复制到另一台服务器；不会改写原始请求。

权威文件位于 `website/data/action_inbox/events/`，一条事件一个 `0600` JSON 文件。跨服务器复制失败时进入 `website/data/shared_state_outbox/inbox/` 自动重试。不要手工改事件文件，也不要在工单或回复中公开邮箱、投诉正文等个人信息。

以下邮箱 Markdown、投诉 JSONL 和通知中心继续保留，是本服务器的兼容日志和排障材料；它们不再承担跨服务器待办去重与处理状态同步。
...
**处理方式：**
1. 优先在 `/ob` 可靠待处理箱查看并更新状态；需要核对旧格式时再打开 `email_requests.md`
...
**处理流程：**
1. 在 `/ob` 可靠待处理箱查看投诉及其来源服务器
2. 需要核对旧格式时打开 `website/data/complaints/` 找到对应记录
3. 根据投诉内容进行调查和处理
4. 如有邮箱，通过邮箱回复处理结果
5. 在通知中心中标记已处理
