...
| 房间计分 PK | `/score-pk` | 无 | `website/templates/pk_score.html` | `/api/pk-score/verify`、`/api/pk-score/data` | 复用 `SCORE_GIFTS_PASSWORD`；`X-PK-Score-Password` | fan-hub `room_record/pk_scores/current.json`；展示双方 17:15 后新增分、基础分、累计分、差值和明细；首轮只发布腾讯云，阿里云数据同步待腾讯云页面验收 |
