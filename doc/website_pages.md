...
| 计分礼物管理 | `/score-gifts` | `/sg`、`/score` | `website/templates/score_gifts.html` | `/api/score-gifts/verify`、`/api/score-gifts/data`、`/api/score-gifts/summary`、`/api/score-gifts/export.xlsx`、`/api/score-gifts/sender-export.xlsx`、`/api/score-gifts/business-review` | `SCORE_GIFTS_PASSWORD`，默认复用 `GIFT_REPLIES_PASSWORD`；`X-Score-Gifts-Password` | `score_gifts.json` 为派生展示数据；送礼用户导出包含用户汇总与逐笔投分明细；`live_business_fulfillments.json` 为版本化共享业务状态 |
