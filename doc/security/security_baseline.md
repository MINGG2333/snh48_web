...
| 可靠待处理箱与客服聊天来源审计 | 投诉、QA 邮箱请求、客服聊天消息和处理状态采用一事件一文件，权限 `0600`；每条事件记录腾讯云/阿里云来源，客服识别码只保存 SHA-256 会话编号，`/ob` 需密码后展示和回复 | 避免并发 JSONL 同步丢请求，同时让管理员区分请求入口并支持双向客服消息 | 事件含邮箱、投诉或聊天正文，不得进 Git、静态目录、公开日志或诊断输出；公开客服接口按 IP 限速，识别码是访问凭据，用户需自行保管 |
