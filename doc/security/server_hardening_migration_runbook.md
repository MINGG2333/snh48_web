...
本次腾讯云与阿里云整改验证了以下顺序可行：先验证全部公钥和带外入口，再关闭密码 SSH；先准备 ACL/运行目录，再切非 root systemd；先 `nginx -t`，再 reload；跨云网站密钥使用强制命令；公开副本图片代理使用回环监听和动态用户；最后从公网验证未知 Host、后端端口、缓存和安全头。
