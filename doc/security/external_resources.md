...
|--------|----------|--------------|----------|
| `8899` 公网直连图片代理 | 生产安全组只开放 `80/443/22`，公网直连阿里云 8899 实测超时；阿里云进程另只监听 `127.0.0.1:8899` | 无影响 | 持续验证安全组和公网 `8899` 不可达 |
| `/image-proxy/` 经 `443` 被刷流量 | 腾讯云和阿里云 Nginx 已启用共享缓存、缓存锁、stale 缓存、后台更新、7 天浏览器缓存、`X-Cache-Status` 和温和 IP 限速；阿里云真实图片已实测 `MISS` 后 `HIT` | 正常用户多图加载保留 `burst=120`，重复访问同图会更快；首次访问新图仍会立即请求上游 | 持续观察 429、缓存盘占用和上游 403/429 |
...
| `flip_data/web/flip_cards.json` | `answerMediaUrl` / 官方下载链接 | `/api/flip-cards/data` | `mp4.48.cn` 等口袋48媒体 CDN | 本地媒体缺失时浏览器播放 CDN 媒体；用户点击“官方下载” | 翻牌页本身需要密码；优先使用本地 `flip_data/audio|video`，外链只作为下载或缺失兜底；CSP 当前允许 HTTPS 媒体 |
...
- 监听地址由 `WEIBO_IMG_PROXY_BIND` 控制；腾讯云因 Nginx 私网回源保留内网监听，阿里云 systemd unit 固定 `127.0.0.1:8899`。两端安全组均不公网放行 8899。
- 脚本向浏览器返回 `Cache-Control: public, max-age=86400`；网站 Nginx 线上配置已覆盖为 7 天浏览器缓存，并启用 `proxy_cache`、`proxy_cache_lock`、stale 缓存和后台更新。不同用户访问同一图片时，首个请求后会优先命中本站缓存。
- 脚本使用 `ThreadingHTTPServer` 和 16 请求有界并发；上游超时 15 秒，响应上限 20 MiB，且只允许 JPEG/PNG/WebP/GIF/AVIF。IP 限速和访问日志由公网入口 Nginx 管理。

结论：

- 代理已经规避了最危险的“任意 URL 开放代理/SSRF”形态，因为上游域名被写死为 `wx1.sinaimg.cn`。
- 当前主要风险转为“经 `443` 暴露的 `/image-proxy/` 被当作微博图片中转站刷流量”：消耗本站带宽、耗尽有界代理并发、放大对 `wx1.sinaimg.cn` 的请求量，进而导致代理 IP 被新浪限流或封禁。
...
|--------|----------|----------|----------------|------|
| `/image-proxy/` 被刷流量 | 公网用户可经 `443` 访问 `/image-proxy/...`，打满本站带宽和代理进程；大量请求转发到 `wx1.sinaimg.cn`，导致代理服务器出口 IP 被上游限流或封禁 | 已实现：1. 云安全组不公网放行 `8899`，阿里云还只监听 `127.0.0.1`；2. Nginx 共享缓存、缓存锁、stale/后台更新，腾讯云 `3GB`、阿里云 `10GB`；3. 7 天浏览器缓存；4. 预热脚本；5. 温和 `limit_req`；6. 16 并发、20 MiB 和图片 MIME 限制 | 缓存会增加磁盘占用；预热会产生少量主动请求；温和限速对正常多图加载影响低，但仍需观察 429 | 阿里云已实测真实图片 `MISS` 后 `HIT`；持续观察磁盘和上游错误 |
