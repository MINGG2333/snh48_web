...
线上自动同步在阿里云执行：cron 每分钟运行 `deploy/sync-from-tencent-if-changed.sh`，通过 SSH 分组检查腾讯云源数据指纹，只有变化时调用 `deploy/sync-from-tencent.sh` 从腾讯云主动拉取对应分组。`deploy/sync-to-aliyun.sh`、`deploy/sync-to-aliyun-if-changed.sh` 和 `deploy/sync-to-aliyun-loop.sh` 只作为腾讯云临时手动推送兜底，不应放回腾讯云生产 cron 或常驻进程。`deploy.py sync-data` 是本地手动触发入口。拉取和手动推送 shell 脚本在每次 rsync 接收时强制 `root:snh48-web`、目录 `0750`、文件 `0640`，因此原子替换文件后网站进程仍保持只读访问；不能改回继承腾讯云 `root:root 0640` 的接收方式。三个 `core` 单文件（事件主文件、兼容副本、社交时间轴）采用显式存在性检查：源文件确认不存在时删除阿里云对应文件，SSH 检查失败则不删除；这样源端删除不会留下陈旧副本。`room_voice_replays/` 在三个入口中都采用 payload 先同步、`manifest.json` 临时文件原子改名、旧 payload 最后清理的发布顺序，避免大音频传输期间网站提前看到半个新会话。管理员新增的事件和行程统一写入 fan-hub 事件/行程 CSV 并按 `core` 单向拉取。四个可写业务状态不进入普通分组同步，也不走 Git；即时一致性由 `doc/shared_runtime_state.md` 的腾讯云权威提交、阿里云操作转发、版本复制和持久 outbox 保证。计分礼物目录的其他只读派生文件继续走 `dynamic`，但所有 rsync 入口都排除可写的 `live_business_fulfillments.json` 和 `.*.lock`。已退役的 `website/data/manual_events.csv` 不加入任何同步源，两个同步入口会主动清理阿里云旧副本。
...
- 网页账号管理继续受翻牌密码 Cookie 保护，POST 还要求同源；手机号和验证码只经本机 Unix Socket 请求体进入独立的 `snh48-privileged-bridge-flip.service`，手机号只进入 fan-hub 本机短期 `0600` 会话，验证码不落盘，Token 只进入 `config/accounts.json`。网站服务本身没有 sudo 或 fan-hub 写权限。翻牌桥通过 `/tmp/snh48-fan-hub-flip-web-rate.lock`、`/tmp/snh48-fan-hub-flip-update.lock`、`/tmp/snh48-fan-hub-transcription.lock` 和 `/tmp/snh48-fan-hub-flip-accounts-manifest.lock` 分别共享短信限速、翻牌更新、本地模型转录及账号清单提交互斥；四个文件必须预创建为 `root:root 0600`，服务单元只放行这些文件而不是整个 `/tmp`。PyTorch 的 `TMPDIR` 固定为 `/home/snh48-fan-hub/notifications/flip_web_admin/tmp` 私有目录。翻牌和房间语音转录共用 fan-hub 专用 `/home/snh48-fan-hub/venv-transcription`，不得重新依赖网站虚拟环境。阿里云只同步脱敏 `web/`、账号级音频和视频，不同步 `metadata/`、`transcripts/`、登录会话或任务日志。
...
- 鉴权：进入公开浏览和提交均使用 `MEMORIES_VIEW_PASSWORD` / `X-Memories-Password`，提交另受 `MEMORIES_SUBMIT_ENABLED`、基础审核和 IP 限速约束；应援会模式使用 `MEMORIES_FANCLUB_PASSWORD` 和 `X-Memories-Fanclub-Password`；本人模式使用 `MEMORIES_IDOL_PASSWORD` 和 `X-Memories-Idol-Password`；`/api/memories/verify` 是公开浏览和提交前的轻量密码验证入口
...
- 两个域名都开放通过访问密码的提交；提交只进入基础审核通过或待人工审核状态，确认和隐藏仍由应援会/本人管理模式完成。阿里云把写操作转发给腾讯云统一串行提交，随后接收相同 revision；普通 `core` 拉取明确不包含此文件。种子脚本也通过同一锁内合并入口写入，不能直接覆盖文件。
...
SITE_MAINTENANCE_MODE=false（迁移窗口设为 true 时仅阻断共享状态/可靠待办写入）
SITE_MAINTENANCE_MESSAGE=迁移维护提示语（可选）
SITE_MAINTENANCE_RETRY_AFTER_SECONDS=300
SHARED_STATE_NODE_LABEL=当前节点在 /ob 中的显示名（可选）
SHARED_STATE_NODE_LABELS_JSON=多节点显示名 JSON 覆盖（可选）
