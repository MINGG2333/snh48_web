...
本文记录网站页面开发时的用户行为追踪要求。新增页面、管理页、短入口或重要交互时，应在开发阶段同步设计追踪，不要等页面上线后再补。页面可见文案与模板测试的同步规则见 `doc/codex/template_test_maintenance.md`。
...
- OB API 同时返回 `ip_network_graph` 供 3D 管理视图使用：IP 节点包含成员档案/旧会话，IP 对之间的边按共同成员集合交集计权，并保留 `historical_only` 置信度标记。该图是静态关系汇总，不是时间轨迹，不改变任何访客、会话或页面访问统计；前端加载失败时回退到二维 IP 节点图和关联组列表。
- OB API 可在鉴权请求期间通过本机 DB-IP Lite MMDB 为历史 IP 临时返回粗略国家、省级地区或城市标签，供用户记录筛选、IP 节点和历史 IP 摘要展示。标签不得写回 `visitor_page_views.jsonl`，不得包含经纬度，不得追加到带时间的逐次访问行，也不得调用逐 IP 外部查询服务；地区库缺失时禁用地区筛选，不能影响其他 OB 功能。
- `geoip` 状态中的 IP 数量必须按“唯一 IP 值 / 可查询公网 IP / 已命中 IP / 跳过或未命中 IP”区分，不得把地区命中数写成访客数、访问次数或 IP 关联条目数；未命中的 IP 仍必须保留在原有 OB 记录中。
