...
2026-09-04 04:05 双服务器观测与日志留存机制上线：网站提交 `ae517a9` 已部署到阿里云公开站，`snh48-aliyun` 主 PID `3844437`、`active/running`、`NRestarts=0`；腾讯云非公开站已通过既有跨云 SSH 快进到同一提交，`snh48-web` 主 PID `3994901`、`active/running`、`NRestarts=0`。两端均启用并启动 `snh48-website-metrics.timer`（每 5 分钟）和 `snh48-website-log-archive.timer`（每日 03:20），最近一次 metrics/archive oneshot 均 `Result=success`。统计分别写入 `/var/lib/snh48-web/metrics/tencent/` 和 `/var/lib/snh48-web/metrics/aliyun/`；首个快照显示腾讯云 2 逻辑 CPU、约 1.92 GiB 内存、根盘 50 GiB 已用 89.51%（可用约 5.24 GiB），阿里云 4 逻辑 CPU、约 7.10 GiB 内存、根盘 68.53 GiB 已用 44.09%（可用约 35.33 GiB）。当前 Nginx 日志总量腾讯云约 600 KiB、阿里云约 1.34 MiB，均未达到 1 GiB 归档阈值，未删除文件；腾讯云可使用 fan-hub 私有 COS 配置，阿里云因不复制敏感凭据保持 fail-closed。观测脚本、systemd unit、logrotate 高保留规则和两种迁移边界文档已提交；阿里云首页、时间轴及部署标准烟测返回 200，预先存在的未跟踪运行文件保持原样。
