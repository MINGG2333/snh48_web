...
def undo_latest_ignored_gift_batch(
    ...
    with _ignore_lock:
        try:
            mutation = execute_mutation("room_ignore", "undo_latest", {})
        except SharedStatePeerError as exc:
            raise HTTPException(status_code=exc.status_code, detail=exc.detail) from exc
        except SharedStateError as exc:
            raise HTTPException(
        ...
        _, new_summary = _load_dataset()
        result = mutation.get("result") if isinstance(mutation.get("result"), dict) else {}
        return _summary_payload(new_summary) | {"undone_batch": result.get("undone_batch") or {}}


def _ignore_latest_mutator(
    state: dict[str, Any], _payload: dict[str, Any]
    ...
    state["version"] = 2
    state["ignored_batches"] = ignored_batches
    return state, {"ignored_batch": ignored_batch}


def _undo_latest_mutator(
    state: dict[str, Any], _payload: dict[str, Any]
) -> tuple[dict[str, Any], dict[str, Any]]:
    ignored_batches = _ignored_batches(state)
    ...
    state["version"] = 2
    state["ignored_batches"] = ignored_batches
    return state, {"undone_batch": undone_batch}


register_mutator("room_ignore", "ignore_latest", _ignore_latest_mutator)
register_mutator("room_ignore", "undo_latest", _undo_latest_mutator)
