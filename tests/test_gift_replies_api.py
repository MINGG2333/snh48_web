...
from website.gift_replies_api import _group_rows_by_sender, get_gift_reply_senders, router
...
class GiftReplySendersApiTests(unittest.TestCase):
    ...
    def call_endpoint(self, *, status: str = "unreplied", sender: str = "") -> dict[str, object]:
    ...
    def test_defaults_to_senders_with_unreplied_but_returns_full_history(self) -> None:
        ...
        self.assertEqual([item["gift_message_id"] for item in payload["items"][0]["items"]], ["new", "old"])
    ...
    def test_router_registers_password_protected_sender_endpoint(self) -> None:
        route = next(route for route in router.routes if route.path == "/api/gift-replies/senders")
...
class GiftReplySendersTemplateTests(unittest.TestCase):
    def test_template_uses_dom_rendering_and_tracks_key_actions(self) -> None:
        ...
        self.assertIn('details.open = true', template)
        ...
        self.assertIn('href="/gift-replies/senders"', room_template)
