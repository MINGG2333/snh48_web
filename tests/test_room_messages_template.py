...
class RoomMessagesTemplateTests(unittest.TestCase):
    def test_room_controls_keep_mobile_filter_and_gift_reply_visibility(self) -> None:
        ...
        self.assertIn('function scrollToLatestMessage()', template)
        self.assertIn('setCurrentScrollTop(Math.max(0, currentScrollHeight() - currentClientHeight()))', template)
